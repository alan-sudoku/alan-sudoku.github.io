...
|:---|:---|:---|:---|
| $C_- $, $C_+$ (subscript) | Abstract / concept-level | [SIRC_principles.md](SIRC_principles.md) | Concept structure: $C_-$ is the negative boundary the sender encodes; $C_+$ is the invariant content the receiver reconstructs. Applies to all SIRC transmissions. |
...
Note: $C$ in SIRC denotes the abstract Concept structure, not Shannon channel capacity $C$ or Kolmogorov complexity $C(x)$.

### Entries

| Symbol | Term | Definition |
|:---|:---|:---|
| $C_-$ | Negative boundary (abstract) | The negative boundary the sender encodes — what a concept excludes. Carried by failed probes, retracted claims, and closed exploration paths. Constrains the receiver's search space by eliminating what the concept cannot be. At document level: the retraction log is the $C_-$ of the principles document. |
| $C_+$ | Positive boundary (abstract) | The invariant content that survives audit — what the receiver reconstructs. At document level: [SIRC_principles.md](SIRC_principles.md) is the $C_+$ of the document pair. |
